...
- Works O.K. as a reference material, but I would not recommend reading it as a book from start to finish
...
- Even harder to read than the SOLPS manual
...
- Length: 5 pages, difficulty: mediu
...
/// note | This is Kateřina's darling topic
Katka's had an article on heat flux limiting in the making since 2021, that's why there's so much stuff on it here. Hopefully after she's had her third baby, she'll finally write it!
///
...
-  Textbook-like review of the physics of heat flux limiters. Go-to material when seeking to learn something about these pesky input parameters. The first paragraph in section *1.3 Historical overview* deserves a medal for its blatant honesty.
...
- Modelled the linear device PISCES-A with a homebrew 1D code, performed a sensitivity study of the electron heat flux limiter, found a big impact on $n_e$ and $T_e$ and concluded the limiter value is bullshit, especially with predictive simulations. Lesson learned: do your sensitivity studies.
...
- Equations used for solving the plasma state in B2.5. There are literally only equations, nearly no description of what the quantities are or mean. Recommended only for hardcore fans of equations. Forget `Ctrl+F`. Decode it according to:
...
Theses are long and intimidating to read. Yet they are usually written by modestly advanced beginners, so they are likely to brilliant explanations. For each thesis, highlight what you think a beginner might find helpful.

**[Katka's PhD thesis study](../files/Katkas_PhD_thesis_study.pdf)<span class="material-symbols-outlined">download</span> (2021)**

- Highlight: Chapter 2 *The SOLPS-ITER code* gives a beginner-friendly introduction to SOLPS and running it. This eventually became the tutorial collection [My first SOLPS-ITER simulation](../my_first_simulation/Creating_a_new_SOLPS-ITER_simulation.md).
/// warning | Work of utter desperation
Katka's PhD thesis study is not particularly good, but she was finishing it very sick, pregnant with her first child, and half considering quitting fusion. It's a work of grit and desperation, and it shall not be forgotten.
///
